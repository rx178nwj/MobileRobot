...
| R2 | コンテナ起動・network_mode:host 確認 | 手動 | 未実装 |
| R3 | colcon ビルド成功確認 | 手動 | 未実装 |
| R4 | ノード起動確認 (4 ノード) | 手動 | 未実装 |
| R5 | /odom 発行確認 (50 Hz) | 手動 | 未実装 |
| R6 | TF ツリー確認 (odom→base_footprint→base_link) | 手動 | 未実装 |
| R7 | /cmd_vel → モーター エンドツーエンド | 手動 | 未実装 |
| R8 | /camera/image_raw/compressed 発行確認 | 手動 | 未実装 |
| R9 | edge_services 再起動後の WebSocket 自動再接続 | 手動 | 未実装 |
| R10 | ノード respawn 動作確認 | 手動 | 未実装 |
...
**目的**: `ws_edge_bringup.launch.py` により所定の 4 ノードが全起動するか確認する。
...
**合否基準**:
- 再起動後 10 秒以内に `/odom` / `/camera/image_raw/compressed` が復帰すること
...
| R2 | コンテナ起動・host ネットワーク確認 | 未実装 |
| R3 | colcon ビルド成功確認 | 未実装 |
| R4 | ノード起動確認 (4 ノード) | 未実装 |
| R5 | /odom 発行確認 (50 Hz) | 未実装 |
| R6 | TF ツリー確認 | 未実装 |
| R7 | /cmd_vel → モーター E2E | 未実装 |
| R8 | /camera/image_raw/compressed 発行確認 | 未実装 |
| R9 | edge_services 再起動後 WS 自動再接続 | 未実装 |
| R10 | ノード respawn 動作確認 | 未実装 |
